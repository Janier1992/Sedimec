  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 flex-1 bg-slate-50/50">
          ...
          {isLoading && !report ? (
            <div className="py-20 text-center space-y-3">
              ...
              <p className="text-xs font-semibold text-slate-600">Auditando consistencia matemática de saldos y registros...</p>
            </div>
          ) : report ? (
            <>
              ...
              <div className={`p-5 rounded-2xl border transition-all ${
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider ${
                          isHealthy ? 'bg-emerald-200 text-emerald-900' : 'bg-rose-200 text-rose-900'
                        }`}>
                  ...
                  <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch md:items-center gap-2 shrink-0">
                    {!isHealthy ? (
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={loadReport}
                          className="flex items-center justify-center gap-1.5 px-3 py-2 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl text-xs font-semibold shadow-xs transition-colors cursor-pointer"
                        >
                ...
                {!isAdmin && !isHealthy && (
                  <div className="mt-3 pt-3 border-t border-rose-200/60 flex items-center gap-2 text-xs text-rose-800 font-medium">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Movimientos Auditados</span>
                  <div className="text-xl font-bold font-mono text-slate-900 mt-0.5">{report.totalMovimientosAuditados}</div>
                  <span className="text-[10px] text-slate-400">Recepciones + Despachos</span>
                </div>
                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Referencias Cruzadas</span>
                  <div className="text-xl font-bold font-mono text-slate-900 mt-0.5">{report.totalReferenciasCruzadas}</div>
                  <span className="text-[10px] text-slate-400">SKUs / Dimensiones</span>
                </div>
                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Piezas Físicas</span>
                  <div className="text-xl font-bold font-mono text-slate-900 mt-0.5">{report.metricasAuditoria.totalPiezasFisicasVerificadas}</div>
                  <span className="text-[10px] text-slate-400">Unidades totales</span>
                </div>
                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Tiempo de Análisis</span>
                  <div className="text-xl font-bold font-mono text-emerald-700 mt-0.5">{report.metricasAuditoria.tiempoEjecucionMs} ms</div>
              ...
              {activeTab === 'inconsistencias' && (
                <div className="space-y-3">
                  {report.detalles.length === 0 ? (
                    <div className="bg-white rounded-xl p-8 text-center border border-slate-200 shadow-xs space-y-2">
                      <CheckCircle2 className="h-10 w-10 text-emerald-500 mx-auto" />
                      <h4 className="text-sm font-bold text-slate-900">No hay inconsistencias detectadas</h4>
                    ...
                    <div className="space-y-3">
                      {report.detalles.map((inc) => (
                        <div
                          key={inc.id}
                          className="bg-white rounded-xl p-4 border border-rose-200 shadow-xs space-y-3"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-bold text-slate-900">{inc.claseEquipo}</span>
                          ...
                          <p className="text-xs text-slate-700 leading-relaxed">{inc.descripcion}</p>

                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-2.5 bg-slate-50 rounded-lg text-xs font-mono">
                            <div>
                              <span className="text-slate-400 text-[10px] block uppercase font-sans font-medium">Valor Registrado / Asentado</span>
                              <span className="font-bold text-rose-700">{inc.valorRegistrado}</span>
                            </div>
                            <div>
                              <span className="text-slate-400 text-[10px] block uppercase font-sans font-medium">Valor Teórico Auditado (Sumatoria)</span>
                              <span className="font-bold text-emerald-700">{inc.valorCalculado}</span>
                            </div>
                          </div>

                          {inc.detallesTecnicos && (
                            <p className="text-[11px] text-slate-500 font-mono bg-white p-2 rounded border border-slate-100">
                              {inc.detallesTecnicos}
                            </p>
                          )}
                          ...
                          <div className="flex items-center justify-between pt-1 border-t border-slate-100 text-xs text-slate-600">
                            <div className="flex items-center gap-1 text-[11px]">
                              ...
                              <span className="text-slate-500">Acción recomendada:</span>
                              <span className="font-medium text-slate-800">{inc.accionRecomendada}</span>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              {activeTab === 'reglas' && (
                <div className="space-y-2.5">
                  {report.reglasAuditoria.map((regla) => (
                    <div
                      key={regla.id}
                      className={`p-3.5 bg-white rounded-xl border shadow-xs flex items-start justify-between gap-3 ${
                        regla.aprobada ? 'border-slate-200' : 'border-rose-200 bg-rose-50/20'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        <div className={`p-1.5 rounded-lg shrink-0 mt-0.5 ${
                          regla.aprobada ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                        }`}>
                          {regla.aprobada ? <CheckCircle2 className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
                        </div>
                        <div>
                          <h4 className="text-xs font-bold text-slate-900">{regla.nombre}</h4>
                          <p className="text-[11px] text-slate-500 mt-0.5 leading-relaxed">{regla.descripcion}</p>
                      ...
                      <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                        regla.aprobada ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                      }`}>
                ...
                <div className="space-y-4">
                  {appliedAdjustments.length > 0 && (
                    <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200 text-xs text-emerald-900 space-y-2">
                      ...
                      <ul className="list-disc list-inside space-y-1 text-emerald-800 font-mono text-[11px]">
                  ...
                  <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-xs space-y-4 font-sans">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-xs font-bold text-emerald-700 font-mono">100% AUDITADO</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <span className="font-bold text-slate-800">Sedimec S.A.S. (Patio Central)</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px] uppercase">Auditor Responsable</span>
                        <span className="font-bold text-slate-800">{report.metricasAuditoria.verificadoPor}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="font-mono text-slate-700">6 de 6 reglas de negocio ASTM/ISO</span>
                      </div>
                    </div>

                    <div className="p-3 bg-slate-50 rounded-lg text-[11px] text-slate-600 leading-relaxed border border-slate-200/60">
        ...
        {/* ── FOOTER DE ACCIONES ── */}
        <div className="px-5 py-3.5 bg-white border-t border-slate-200 flex items-center justify-between text-xs shrink-0">
